...
REM ===============================================
REM     Compilateur Global Kraphtea-Mods Amélioré
REM     Version: 1.1.0
REM ===============================================

title Compilation de tous les mods Kraphtea
...
REM Dossier de base (ajuste si nécessaire)
set BASE_DIR=%CD%
echo [INFO] Dossier de base: %BASE_DIR%
...
for %%m in (%MODS%) do (
    ...
    if not exist "%BASE_DIR%\!MOD_NAME!" (
        echo [ERREUR] Le dossier du mod !MOD_NAME! n'existe pas
        echo          Recherché dans: %BASE_DIR%\!MOD_NAME!
        set /a SKIPPED_MODS+=1
        echo.
        goto :continue_loop
    )
    
    if not exist "%BASE_DIR%\!MOD_NAME!\build.gradle" (
        echo [ERREUR] build.gradle introuvable pour !MOD_NAME!
        echo          Recherché dans: %BASE_DIR%\!MOD_NAME!\build.gradle
        set /a SKIPPED_MODS+=1
        echo.
        goto :continue_loop
    )
    
    cd "%BASE_DIR%\!MOD_NAME!"
...
if %FAILED_MODS% EQU 0 (
    if %SKIPPED_MODS% EQU 0 (
        echo Tous les mods ont été compilés avec succès!
    ) else (
        echo Attention: %SKIPPED_MODS% mod(s) ont été ignorés.
